...
local wallpaper_cmd = "find " .. wallpaper_dir .. " -type f -name '*'  -print0 | shuf -n1 -z | xargs -0 feh --bg-scale"
...
vicious.register(batwidget, vicious.widgets.bat,
	function (widget, args)
		if args[2] == 0 then return ""
			...
			return "<span color='white'>"..  args[1] ..  args[2] .."%</span>"
		end
	end, 61, "BAT1"
)
-- }}}
...
globalkeys = awful.util.table.join(
    ...
    -- valume Key
    awful.key({ modkey,           }, "Up", function () exec("amixer -q set Master 2dB+", false) vicious.force({volbar, volwidget}) end),
    awful.key({ modkey,           }, "Down", function () exec("amixer -q set Master 2dB-", false) vicious.force({volbar, volwidget}) end),

    --Backlight Brightness
    awful.key({ modkey, "Mod1"   }, "Up", function () exec("xbacklight +10", false)    end),
    awful.key({ modkey, "Mod1"   }, "Down", function () exec("xbacklight -10", false)    end),
    ...
    awful.key({ modkey, "Shift"   }, "a",     function () os.execute(wallpaper_cmd)     end),
 
    awful.key({ modkey }, "b", function ()
         wibox[mouse.screen].visible = not wibox[mouse.screen].visible
    end),

    -- Alt + Right Left switches the current keyboard layout
